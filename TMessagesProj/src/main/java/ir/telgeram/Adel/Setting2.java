...
public class Setting2
{
	...
	public static boolean getVoiceReq()
	{
		setupSetting();
		return pref.getBoolean("voicereq", false);
	}

	public static void setVoiceReq(Boolean on)
	{
		setupSetting();
		editor.putBoolean("voicereq", on);
		editor.commit();
	}
	public static boolean getStickerReq()
	{
		setupSetting();
		return pref.getBoolean("stickerreq", false);
	}

	public static void setStickerReq(Boolean on)
	{
		setupSetting();
		editor.putBoolean("stickerreq", on);
		editor.commit();
	}
	...
	{
		boolean send = Setting2.TabisShowed(tab);
		if (send)
		{
			Setting2.hideTab(tab);
			return false;
		}
		else
		{
			Setting2.ShowTab(tab);
			return true;
		}
	}
